...
import com.bizzan.bitrade.vo.MemberStatusVo;
...
public class AdvertiseController extends BaseController {
    ...
    public MessageResult create(@Valid Advertise advertise, BindingResult bindingResult,
        ...
        List<BusinessAuthApply> businessCancelApplies = businessAuthApplyService.findByMember(member1);
        MemberStatusVo memberStatusVo = new MemberStatusVo();
        memberStatusVo.setType("0");
        String type = "0";
        ...
        OtcOrderExpire expire = otcOrderExpireService.findById(member.getId(), DateUtil.getDate());
        if (expire != null) {
...
//        if (allow == 1) {
        ...
        if (advertiseType == AdvertiseType.SELL) {
            Assert.isTrue(type.equals("2"), msService.getMessage("NO_BUSINESS"));
        }
        List<BusinessCancelApply> applyServiceByMemberAndStaus = businessCancelApplyService.findByMemberAndStaus(member1, CertifiedBusinessStatus.CANCEL_AUTH);
...
//        }
        ...
        advertise.setTimeExceed(Integer.parseInt(advertuse_exchange));
        advertise.setTimeLimit(Integer.parseInt(advertuse_pay));
        advertise.setRealNumber(advertise.getNumber());
    ...
    public MessageResult putOnShelves(long id, @SessionAttribute(SESSION_MEMBER) AuthMember authMember) throws InformationExpiredException {
        ...
        if (advertise.getAdvertiseType().equals(AdvertiseType.SELL)) {
            ...
            BigDecimal remainAmount = advertise.getRemainAmount();
            if (type.equals("1")) {
                commission = mulRound(remainAmount, getRate(otcCoin.getJyRateCommonSell()));
            } else if (type.equals("2")) {
                commission = mulRound(remainAmount, getRate(otcCoin.getJyRateSell()));
            } else {
                commission = mulRound(remainAmount, getRate(otcCoin.getJyRate()));
            }
            ...
            Assert.isTrue(compare(memberWallet.getBalance(), BigDecimalUtils.add(commission, remainAmount)), msService.getMessage("INSUFFICIENT_BALANCE"));
            ...
            MessageResult result = memberWalletService.freezeBalance(memberWallet, BigDecimalUtils.add(commission, remainAmount));
            if (result.getCode() != 0) {
    ...
    public MessageResult putOffShelves(long id, @SessionAttribute(SESSION_MEMBER) AuthMember authMember) throws InformationExpiredException {
        ...
        advertiseService.putOffShelves(id, memberId);
    ...
    public MessageResult memberPhone(@PathVariable("phone") String phone, @PathVariable("type") Integer type) {
        Member member = memberService.findByPhone(phone);
        if (member != null) {
            MemberAdvertiseInfo memberAdvertise = advertiseService.getMemberAdvertise(member, coins.getCoins(), type);
            MessageResult result = MessageResult.success();
            result.setData(memberAdvertise);
            return result;
        } else {
    ...
    private void checkAmount(AdvertiseType advertiseType, Advertise advertise, OtcCoin otcCoin, Member member) {
        if (advertiseType.equals(AdvertiseType.SELL)) {
            ...
            if (type.equals("1")) {
                commission = mulRound(number, getRate(otcCoin.getJyRateCommonSell()));
            } else if (type.equals("2")) {
                commission = mulRound(number, getRate(otcCoin.getJyRateSell()));
            } else {
                commission = mulRound(number, getRate(otcCoin.getJyRate()));
            }

            Assert.isTrue(compare(memberWallet.getBalance(), BigDecimalUtils.add(commission, advertise.getNumber())), msService.getMessage("INSUFFICIENT_BALANCE"));
        } else {
